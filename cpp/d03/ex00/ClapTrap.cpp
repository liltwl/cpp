...
ClapTrap::ClapTrap(const std::string &pName) :  name(pName) {
	std::cout << "ClapTrap constructor called" << std::endl;
	Hitp = 10;
	Energyp = 10;
	Attackp = 0;
...
void ClapTrap::attack(const std::string &target) {
	...
	std::cout << Hitp << " points of damage!" << std::endl;
}
...
std::ostream &operator<<(std::ostream &out, ClapTrap const & clapTrap) {
	out << "Name: " << clapTrap.name << " HP: " << clapTrap.Hitp
		<< " EP: " << clapTrap.Energyp << " Attack Damage: " << clapTrap.Attackp << std::endl;
	return out;
}